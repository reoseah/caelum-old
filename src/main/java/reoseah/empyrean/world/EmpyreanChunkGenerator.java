...
import reoseah.empyrean.world.biomes.EmpyreanBiome;
import reoseah.empyrean.world.structures.LargeIslandStructureFeature;

public class EmpyreanChunkGenerator extends SurfaceChunkGenerator<EmpyreanChunkGeneratorConfig> {
	...
	public EmpyreanChunkGenerator(IWorld world, BiomeSource biomeSource, EmpyreanChunkGeneratorConfig config) {
	...
	protected PointInfo computePointInfo(int x, int z) {
		...
		for (int dx = -2; dx <= 2; ++dx) {
			for (int dz = -2; dz <= 2; ++dz) {
				...
				float size = (biome instanceof EmpyreanBiome) ? ((EmpyreanBiome) biome).getIslandsSize() : 0;
