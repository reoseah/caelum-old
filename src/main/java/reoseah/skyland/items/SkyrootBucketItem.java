...
public class SkyrootBucketItem extends Item {
				...
				if (this.fluid == Fluids.EMPTY) {
					...
					if (state.getBlock() instanceof FluidDrainable) {
						...
						if (fluid == Fluids.WATER) {
							...
							ItemStack filled = this.getFilledStack(stack, player, SkyItems.SKYROOT_WATER_BUCKET);
							if (!world.isClient) {
	...
	protected ItemStack getEmptiedStack(ItemStack stack, PlayerEntity player) {
		return !player.abilities.creativeMode ? new ItemStack(SkyItems.SKYROOT_BUCKET) : stack;
	}
